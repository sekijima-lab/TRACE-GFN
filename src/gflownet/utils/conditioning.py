...
class TemperatureConditional(Conditional):
    def __init__(self, cfg: Config, rng: np.random.Generator):
        ...
        if tmp_cfg.sample_dist == "gamma":
            ...
            self.upper_bound = tmp_cfg.dist_params[1] # default: 32
    ...
    # SamplingIteratorの__iter__で実行されるメソッド
    def sample(self, n):
        cfg = self.cfg.cond.temperature
        beta = None
        if cfg.sample_dist == "constant":
            # dist_paramsはTemperatureConfigで設定
            # default: [0.5, 32.0] -> seh_reactions.pyのhpsで[64.0]で上書きされる
            assert isinstance(cfg.dist_params[0], float) # Trueの場合のみ↓が実行される
        ...
        return {"beta": torch.tensor(beta), "encoding": beta_enc} # beta: [64.0, 64.0, ..., 64.0], beta_enc: [[0, ...,],[0, ...,],...,[0, ...,]]
        # temperatureをサンプリングしない場合(constant)はbetaが固定されるので、beta_encも固定ベクトルを渡す
        
    # FlatRewardsをRewardScalarにtransform
    def transform(self, cond_info: Dict[str, Tensor], linear_reward: Tensor) -> Tensor:
        scalar_logreward = linear_reward.squeeze().clamp(min=1e-30).log() # flat_rewardのminを10^-30にしてlogを取る, logの値が-infになるのを防ぐ
        if scalar_logreward.shape == torch.Size([]):
            scalar_logreward = scalar_logreward.unsqueeze(0)
