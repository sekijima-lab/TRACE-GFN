...
def beam_decode(v:Vocab, model=None, input_tokens=None, template_idx=None,
                ...
                beam_templates:list=None) -> list[str]:
...
def prob_sampling(v=None, model=None, input_tokens=None, device=None, inf_max_len=None, template_idx=None, beam_templates:list=None, temperature=1.0) -> list[str]:
    '''
    in:
    ...
    if template_idx is not None:
        ...
        if template_idx not in beam_templates:
            beam_width = 5
            nbest = 1
            output, score = beam_decode(v=v, model=model, input_tokens=input_tokens, template_idx=template_idx,
    ...
    with torch.no_grad():
        ...
        log_probabilities = torch.zeros(encoder_input.size(1)).to(device)
        for i in range(inf_max_len - 1):
            tgt_mask = torch.nn.Transformer.generate_square_subsequent_mask(dec_inp.size(0)).to(device)
            logits = model.decode(memory=enc_out, tgt=dec_inp, tgt_mask=tgt_mask, memory_pad_mask=memory_pad_mask) # logits: (seq, batch, vocab)
            dec_out = F.softmax(logits[-1, :, :]/temperature, dim=1) # extract softmax of last token, (batch, vocab)
            # probabilistic sampling
            next_items = torch.multinomial(dec_out, num_samples=1)  # (batch, 1)
            next_items_prob = dec_out[torch.arange(dec_out.size(0)), next_items.squeeze()]
            EOS_indices = (next_items == EOS_token).squeeze()
            # update EOS_dic and log_probabilities
            for j, EOS in enumerate(EOS_indices):
                if EOS_dic[j]:
                    continue
                else:
                    log_probabilities[j] += torch.log(next_items_prob[j])
                    if EOS:
                        EOS_dic[j] = True
            dec_inp = torch.cat([dec_inp, next_items.permute(1, 0)], dim=0) #dec_inp.shape: (seq, batch)
            if sum(list(EOS_dic.values())) == encoder_input.size(1):
                break
        out = dec_inp.permute(1, 0) # (seq, batch) -> (batch, seq)
        output_score_dict = {}
        duplicates = 0
        log_probabilities = log_probabilities.tolist()
        for out, score in zip(out, log_probabilities):
            out_tokens = v.lookup_tokens(out.tolist())
            try:
                eos_idx = out_tokens.index('<eos>')
                out_tokens = out_tokens[1:eos_idx] # remove <bos> and <eos>
                smi = ''.join(out_tokens)
                if smi not in output_score_dict or score > output_score_dict[smi]:
                    output_score_dict[smi] = score
                else:
                    duplicates += 1
            except ValueError:
                continue
        # 重複を除いたoutputsとscoresを取得
        # print(f'{ (duplicates / encoder_input.size(1)) * 100:.2f}% SMILES are duplicated in probabilistic sampling.')
        unique_outputs = list(output_score_dict.keys())
        unique_scores = list(output_score_dict.values())
        return unique_outputs, unique_scores
